...
/**
 * 開発向けシード。
 * - ユーザー 3 名 (alice / bob / charlie) と 1 グループ
...
export async function seedDev(prisma: PrismaClient): Promise<void> {
  ...
  await prisma.subscription.upsert({
    ...
    update: { plan: 'basic' },
    create: { userId: charlie.id, plan: 'basic', status: 'active' },
  });
  ...
  await prisma.groupMember.upsert({
    ...
    update: {},
    create: { userId: charlie.id, groupId: group.id, role: 'member' },
  });
  ...
  const charlieMessageCount = await prisma.message.count({
    where: { groupId: group.id, senderId: charlie.id },
  });
  if (charlieMessageCount === 0) {
    const charlieMessages = [
      'Hi everyone, Charlie here!',
      'Excited to join this channel.',
      'Happy to help out wherever I can.',
    ];
    for (const content of charlieMessages) {
      await prisma.message.create({
        data: { content, type: 'text', senderId: charlie.id, groupId: group.id },
      });
    }
  }
  ...
  await prisma.userFriend.upsert({
    where: { userId_friendId: { userId: alice.id, friendId: charlie.id } },
    update: {},
    create: { userId: alice.id, friendId: charlie.id, status: 'accepted' },
  });
}
