...
export async function middleware(request: NextRequest) {
  ...
  // CSP ヘッダーの追加（本番環境のみ）
  // 開発環境では Next.js dev overlay (HMR / devtool-style-inject) が
  // ノンスなしのインラインスクリプト・スタイルをインジェクトするため CSP を適用しない
  if (!isDev) {
    ...
    const cspHeader = `
      ...
      img-src 'self' blob: data: https://lh3.googleusercontent.com https://avatars.githubusercontent.com https://api.dicebear.com${r2ImgOrigin};
      font-src 'self';
      connect-src 'self';
      object-src 'none';
      base-uri 'self';
